...
if [ ! -d ${stack_base_nfs}/x86_64/generic/modules/$class ]; then
   echo "Warning: module directory for $class does not exist yet, it will be created."
   #exit -1
   #mkdir -p ${stack_base_nfs}/x86_64/generic/modules/$class
fi

for genarch in ${generic_architectures}; do
    echo "Creating software directory for generic software in ${stack_base_nfs}/${genarch}/software"
    mkdir -p ${stack_base_nfs}/${genarch}/software/${software}

    echo "Creating module paths for generic software in ${stack_base_nfs}/${genarch}/modules"
    mkdir -p ${stack_base_nfs}/${genarch}/modules/all/${software}
    mkdir -p ${stack_base_nfs}/${genarch}/modules/${class}/${software}
done

echo "Linking software directory for each architecture"
for arch in ${architectures}; do
   echo "  ${arch}"
   cd ${stack_base_nfs}/${arch}/software
...
for arch in ${architectures}; do
   echo "  ${arch}"
   mkdir -p ${stack_base_nfs}/${arch}/modules/all
   cd ${stack_base_nfs}/${arch}/modules/all
   ln -s ../../../../generic/modules/all/${software} .
   mkdir -p ${stack_base_nfs}/${arch}/modules/${class}
   cd ${stack_base_nfs}/${arch}/modules/${class}
